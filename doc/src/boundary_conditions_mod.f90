module boundary_conditions_mod
   ...
   !---------------------------------------------------------------------------!
      subroutine transform_summation(number_of_channels, jtot_, v_, j_, vp_,   &
         jp_, l_, lp_, channels_level_indices, channels_omega_values,          &
         bf_matrix, sf_element)
      ...
      subroutine bf_to_sf_transformation(number_of_channels,jtot_,             &
         ...
         bf_matrix,sf_matrix)
         !! takes an input matrix in the BF-frame and transforms it to
         !! the SF-frame; calls transsum and pfunc
         !---------------------------------------------------------------------!
         integer(int32), intent(in) :: number_of_channels
            !! size of the basis
         integer(int32), intent(in) :: jtot_
      ...
      subroutine calculate_k_matrix(number_of_channels, log_der_matrix,        &
         ...
         do iopen = 1, number_of_open_channels
            ...
            x = wavenumber*r_
            call riccati_bessel_j(channels_l_values(open_channels_indices(iopen)),&
               x,j_element_,jp_element_)
            diag_j_matrix(iopen,iopen) = (wavenumber)**(-0.5d0)*j_element_
            diag_jp_matrix(iopen,iopen) = (wavenumber)**(0.5d0)*jp_element_

            call riccati_bessel_y(channels_l_values(open_channels_indices(iopen)),&
               x,n_element_,np_element_)
         ...
         do iclosed = 1,number_of_channels-number_of_open_channels
            ...
            x = wavenumber*r_
            call modified_bessel_k_ratio(channels_l_values(closed_channels_indices(iclosed)),x,ratio)
            !------------------------------------------------------------------!
      ...
      subroutine unitarity_check(number_of_open_channels,s_matrix_real,        &
         ...
         do iopen=1, number_of_open_channels
            ...
            do iopen2=1, number_of_open_channels
               unitarychecktmp = unitarychecktmp +                             &
                  s_matrix_real(iopen,iopen2)**2. + s_matrix_imag(iopen,iopen2)**2.
               !---------------------------------------------------------------!
               if (prntlvl.ge.4) then
                  write(line_, "(1X,I5,1X,I5,8X,E15.8,8X,E15.8,8X,E15.8)")     &
                     iopen, iopen2, s_matrix_real(iopen,iopen2)**2.+s_matrix_imag(iopen,iopen2)**2.,  &
